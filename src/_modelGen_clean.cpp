...
{
    //   ball sigma  sigma_perp / parallel   sigma
...
void ModelGenerator::sample_interior_points(std::vector<Eigen::Vector3d>& pore_centers, std::vector<double>& pore_sdfs, std::vector<int>& inside_indices, int pores, std::mt19937& gen)
{
    ...
    for (int idx = 0; idx < grid_num; ++idx) {
        ...
        if (SDF(idx) < out_margin && SDF(idx) > -margin) {   // 
...
void ModelGenerator::show_model()
{
    ...
    viewer.data(id2).show_faces = false;   //  
    ...
    viewer.data(id3).show_lines = false;   //  
    ...
    viewer.data(id4).show_lines = false;   //  
...
std::vector<int> ModelGenerator::find_specified_path(int p_index, int s1, int s2, AdjacencyList adj)
{
    ...
    full_path.insert(full_path.end(), path2.begin() + 1, path2.end()); //   path2  
...
double ModelGenerator::calculate_path_translucency(std::vector<int>& path, bool show_debug)
{
    ...
    // ---------- 1. Angle term----------
    double T_angle;
    const double angle_floor = 0.6; //  _angle
...
double ModelGenerator::cal_kernel_translucency(int p_index, int & max_s1, int & max_s2, std::vector<int>& max_path, AdjacencyList adj, bool debug)  // p 
{
    double ave_perm = 0.0;
    int count_ = 0;
    double max_perm = 0.0;
    ...
    if (!p_bfs.isConnected)  // 
    {
        if(debug_show)
            cout << " " << endl;
        return 0.0;
    }
	//if (adj[p_index].size() < 2 && (!Kernels[p_index].on_surface))    // 0
 //   {
	//	return max_perm;
 //   }
    //   s1, s2  ,   O(K^2)  K   surface_points  
    for (int i = 0; i < surface_kernels.size(); i++) 
    {
        ...
        for (int j = i+1; j < surface_kernels.size(); j++) 
        {
            ...
            path_.insert(path_.end(), path2.begin() + 1, path2.end()); //   path2  
...
double ModelGenerator::line_cross_surface(Eigen::Vector3d p1, Eigen::Vector3d p2, double thres, int sam_num)
{
    ...
    {
        return max(1.0 - abs(center_sdf + 0.05) / 0.1, 0.0); // 1-dis
        //return 1.0 - abs((center_sdf - thres) / thres); // 1-dis
    }
}
...
bool ModelGenerator::replace_edges(int p_index, int replace_e, std::vector<Edge>& tube_edges, AdjacencyList& adj, AdjacencyList& unused_adj)
{
    ...
    for (int candidate_p : unused_adj_new[p_index])
    {
        ...
        double score_add = (delta_score.first + delta_score.second) / cand_edge.length;  // 
        if (delta_score.first > max_delta_score && delta_score.second > -1e-9)  // p_index 
        {
    ...
    if (chosen_cand != -1)
    {
        edge_mst_new[replace_e] = chosen_e; // 
        //update adj_list
        ...
        unused_adj_new[re_edge.from].push_back(re_edge.to);  // 
...
void ModelGenerator::optimize_mst(int opt_times_once, int edge_max, vector<int>& rep_vec, bool debug)
{
    ...
    for (int i = 0; i < kernels_num; i++)
    {
        ...
        if (Adj_list[i].size() < 2 && (!Kernels[i].on_surface)) // 
        {
            std::cout << "===========================================================" << endl<<
    ...
    for(int i = 0; i < kernels_num; i++)
    {
        ...
        if (kernel_translucency[i] < 1e-9 && (!Kernels[i].on_surface)) // 
        {
			int start = -1, end = -1;
            std::vector<int> max_path;
        ...
        while(kernel_translucency[i] < thres_tran && opt_count < opt_times_once && opt_count_total< opt_times_once * kernels_num)
        {
            ...
            if (curr_edge_num < edge_max_num && !add_end && Adj_list[i].size() < Max_degree+1)  // score 
            {
...
void ModelGenerator::optimize_mst2(int itea_max_times, int max_edge, bool iter_add, bool debug) //max_edge = 0 !=0 
{
    std::cout << "--------------------4. Optimizing the connection trees --------------------" << endl;
    ...
    while (ratio < itea_max_times && delta_trans_score>0.01)
    {
        ...
        for (int index = 0; index < kernels_num; index++)
        {
            ...
            if (Adj_list[i].size() >= 2 || Kernels[i].on_surface) // 
            {
                ...
                sort_min2max(sorted_edges);//   ( ),   second  
                ...
                for (int s_index = 0; s_index < sorted_edges.size(); s_index++)
                {
                    ...
                    else
                    {
                        ...
                        for (int candidate_p : unused_adj_new[i])
                        {
                            ...
                            if (delta_score.first > delta_replace_score && delta_score.second > -1e-9)  // p_index 
                            {
            ...
            else if (choose_replace)
            {
                ...
                Tube_edges[replaced_edge_index] = chosen_e2; // 
                //update adj_list
                ...
                Unused_adj_list[re_edge.from].push_back(re_edge.to);  // 
...
double ModelGenerator::generate_tube(const Eigen::Vector3d& p, const GaussianKernel& k1, const GaussianKernel& k2, double iso_level_C, double mid_radius_factor) //  
{
    ...
    Eigen::Vector3d p_proj = c1 + t_proj * line_dir; // p 
...
//     = 1/(2 * sigma^2) 
    ...
    double mu = 10 * mid_radius_factor * avg_omega;  //mid_radius_factor 
...
#pragma omp parallel for 
    ...
    if (Enable_noise)
    {
        ...
        //add_noise_near_isosurface(SDF_gaussian_tubes, GV, Isolevel, 0.66, 0.2, 6.3);  //200 
        //add_noise_near_isosurface(SDF_gaussian_tubes, GV, Isolevel, 0.8, 0.3, 7);  // 300 
        add_noise_near_isosurface(SDF_gaussian_tubes, GV, Isolevel, fine_ratio * noise_amplitude, fine_ratio * band_width, fine_ratio * spatial_frequency);  //100 
    }
...
//#pragma omp parallel for reduction(+:solid_count)
    ...
    //std::cout << "  " << void_centers.size() << "  " << std::endl;
...
void ModelGenerator:: calculate_tube_matrixs(vector<Eigen::Matrix3d>& S_matrixs, std::vector<std::vector<Eigen::Matrix2d>>& W_perp_matrixs)
{
    ...
    for (int k1 = 0; k1 < k_num; k1++) {
        for (int k2 = 0; k2 < k_num; k2++)
        {
            ...
            Eigen::Matrix2d W_perp = S_perp.inverse(); //  _perp^{-1}
...
void ModelGenerator::compare_msc(Eigen::VectorXd SDF_gaussian, int res, int grid_num, double smooth_t)
{
    ...
    std::vector<Vec3> gaussian_centers; //          
    std::vector<Eigen::Vector3d> pore_centers;
...
{
    std::string scriptDir = "D:/VSprojects/TaihuStone/src"; // Python 
    std::string pythonExe = "D:/VSprojects/TaihuStone/limitstl/taihu_env/Scripts/python.exe";
    std::string scriptName = "gpu_topology_optimizer.py"; //  
...
int ModelGenerator::resolveOverlaps3D(std::vector<GaussianKernel>& kernels, int maxIters, double isoValue, double minSigmaPerp, double minSigmaParallel, double tol, double margin, double minScalePerUpdate, bool verbose)
{
    ...
    const double chi = -2.0 * std::log(isoValue); // amplitude=1  -0.5
    ...
    for (int iter = 0; iter < maxIters; ++iter) {
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = i + 1; j < n; ++j) {
                ...
                //  
                // h(n)   s  sigma -> s*sigma  =>   -> s^2   => h -> s h
                //  s*(hi+hj) <= proj - margin - tol
    ...
    double chi)
{
    ...
    Eigen::Vector3d x = ldlt.solve(n_unit); // x =   n
    double nsn = n_unit.dot(x);             // n^T   n
...
void ModelGenerator::removeUnusedNodes(std::vector<GaussianKernel>& k, 
    ...
    // 1.   (Old Index -> New Index)
    // mapping[old_id] = new_id;   -1
    ...
    for (const auto& e : new_edges) {
        ...
        new_adj[e.second].push_back(e.first); //  
    }
